...
/**
 * Stringet készít nullával lezárt karaktersorozatból (C-sztringből)
...
char charAtString(const String& s0, unsigned int idx) {
  if(idx >= s0.len) { // ezt nem teljesen értem miért különbözik a .c -s kódtól, de találgatással eltaláltam és most működik
    throw "Valami hiba";
  }
  return s0.pData[idx];
}
...
String addString(const String& s0, const String& s1) {
  String str;
  str.len = s0.len + s1.len;
  str.pData = new char[str.len + 1];
  strcpy(str.pData, s0.pData);
  strcat(str.pData, s1.pData);
  return str;
}
